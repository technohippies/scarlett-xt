...
export const ChatMessageBase: React.FC<ChatMessageBaseProps> = ({
  ...
  const roleClasses = 
    role === 'user'
      ? 'bg-gray-400 text-gray-900 self-end' // User
      : isError // Check transient error prop
      ? 'bg-red-100 text-red-800 border border-red-200 self-start' // Error style
      : role === 'assistant' || role === 'system' // Handle assistant/system normally
      ? 'bg-gray-300 text-gray-900 self-start' // Assistant/System
      : 'bg-blue-100 text-blue-800 self-start'; // Fallback/other roles (bookmark, flashcard - should not use this base directly)
  ...
  return (
    <div className={`flex w-full my-2 ${alignmentClasses}`}>
      <div className={cn(baseClasses, roleClasses)}>
        ...
        <p className="whitespace-pre-wrap">{content ?? ''}</p> {/* Handle potentially null content */}
      </div>
    </div>
  );
}; 